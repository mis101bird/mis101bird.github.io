...
不論是撰寫前端 Javascript，還是後端 NodeJS，異步操作都是無法避免的，像後端資料請求、處理瀏覽器事件、計時器等...都是異步操作。
異步和我們熟悉的同步不同，當異步執行後，瀏覽器/NodeJS 不會等待當前操作的結果回傳，才執行下一行程式碼，他會繼續執行(無阻塞)，等遠端資料回傳，通知 Javascript 引擎，才呼叫使用者當初指定的 callback 函式做進一步的處理。

以下面程式碼為例。先定義一個函式 getRemoteData，目的是用 Ajax 技術發出GET請求，這個異步函式會在後面的例子不斷被使用到。
...
異步操作需要注意的就是 Callback pattern 的撰寫方式，可以參考下面例子。

這裡程式不會等待 getRemoteData 完成後才繼續執行，而是會馬上接著跑 showStudents。然而，此時 showStudents 的參數 students 的結果還沒回傳，因此造成程式錯誤。
...
這個原因是因為 showStudents 並不是馬上執行，而是等到當時的 grades 回傳後才被呼叫，此時的 student 已經不是當時請求送出時的 student，而是被最後一次給值的 student，也就是 111113。所以我們必須要用 curry 寫法來避免這種問題產生，如下。
